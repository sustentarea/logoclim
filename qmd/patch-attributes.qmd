...
- **`value-12`**: A `list` containing the current value of the climate variable along with the previous 11 values. These values are ordered chronologically, with the most recent value (stored in `value`) at the end of the list.
- **`value-12ma`**: A `number` representing the 12-month moving average of the climate variable, i.e., the mean of the values in the `value-12` list. This attribute is used to smooth out short-term fluctuations in the climate data and highlight longer-term trends.
