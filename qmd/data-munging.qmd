...
::: {.callout-important}
Small datasets may contain data gaps (bleeds) in coastal areas after spatial enveloping operations. Test your processed data in `LogoClim` before implementing it in your model to ensure data integrity.
:::
...
source(here("R", "logoclim_readme.R"))
```
...
shape <-
  ...
  subset(., !.$NAME_1 %in% c("Azores", "Madeira"))
```
...
shape <-
  ...
  subset(., !.$NAME_1 %in% c("Islas Canarias"))
```
...
shape <-
  ...
  subset(., .$NAME_1 %in% european_russia_regions)
```
...
countries <-
  ...
  str_subset("RUS", negate =  TRUE)

shape <-
  world(resolution = 1, path = raw_data_dir) %>%
  subset(., .$GID_0 %in% countries) |>
  union(european_russia_shape)
```
...
# xmin, xmax, ymin, ymax
box <- c(-73.99044997, -28.83594354, -33.75114991, 5.27184108)
```
...
if (!is.null(shape)) {
  shape_x_min <- round(ext(shape)[1])
  shape_x_max <- round(ext(shape)[2])
...
if (!is.null(box)) cropped_ras <- cropped_ras |> crop(box)
```
