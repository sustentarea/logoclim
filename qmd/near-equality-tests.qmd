...
1. **Decimal precision loss**: [Esri ASCII](https://en.wikipedia.org/wiki/Esri_grid) files store values as text with a fixed number of decimal places, rounding continuous values from the original [GeoTIFF](https://en.wikipedia.org/wiki/GeoTIFF).
2. **Spatial interpolation**: When NetLogo's patch grid doesn't perfectly align with the raster resolution, the [GIS extension](https://docs.netlogo.org/gis.html) applies interpolation that may alter cell values.
