...
`LogoClim` integrates [WorldClim](https://www.worldclim.org/) data into [NetLogo](https://www.netlogo.org/) models, requiring the conversion of raster data between formats.

[WorldClim](https://worldclim.org/) provides raster data in [GeoTIFF](https://en.wikipedia.org/wiki/GeoTIFF) format, a high-precision standard for geospatial data with extensive metadata capabilities. However, [NetLogo's GIS extension](https://docs.netlogo.org/gis.html) requires [Esri ASCII](https://en.wikipedia.org/wiki/Esri_grid) format, a simpler text-based format with lower precision and limited metadata.
...
::: {.callout-note}
The [`rspatial`](https://rspatial.org/) [`geodata`](https://rspatial.github.io/geodata/) R package [@hijmans2024] was used to download country shapes from the [GADM](https://gadm.org/) database [@hijmansa].
:::
...
::: {.callout-note}
The [`rotate()`](https://rspatial.github.io/terra/reference/rotate.html) function from the [`terra`](https://rspatial.github.io/terra/) package [@hijmans2026] is used to adjust shapes that cross the [International Date Line](https://en.wikipedia.org/wiki/International_Date_Line) (e.g., Russian territory).
...
::: {.callout-note}
The `dx` parameter specifies the degree and direction of data rotation. Negative values rotate the data to the left, while positive values rotate it to the right. This adjustment is applied only for countries crossing the [International Date Line](https://en.wikipedia.org/wiki/International_Date_Line).
:::
...
```{r}
while (setup$model %in% c("FIO-ESM-2-0", "GFDL-ESM4", "HadGEM3-GC31-LL")) {
  setup <- worldclim_random("fcd")
}
```
...
setup_file <- create_experiment(
  ...
  constants = list(
    ...
    "data-resolution" = names(setup$resolution),
    "climate-variable" = names(setup$variable),
    "bioclimatic-variable" = names(setup$bioclimatic_variable),
...
compare_plots(
  ...
  layer = ifelse(
    setup$variable == "bioc",
    paste0("bio", setup$bioclimatic_variable),
    setup$variable |>
      paste0(str_pad(setup$month, width = 2, pad = "0"))
  ),
  viridis = FALSE
)
```
